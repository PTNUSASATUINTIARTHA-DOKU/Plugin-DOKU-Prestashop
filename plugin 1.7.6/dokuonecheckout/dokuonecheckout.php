...
/*
    ...
    Description : DOKU Onecheckout Payment Gateway for Prestashop 1.7
    Version     : 2.3.0
    Author      : DOKU
...
class DOKUOnecheckout extends PaymentModule
{
	...
	public function hookPaymentOptions($params)
    {
		...
		if (intval($CHANNEL_OPTION) == 1) {
			if ( intval($CHANNEL_OPTION) == 0 && intval($USE_INSTALLMENT) == 1 && $TOTAL_AMOUNT > $MIN_INSTALLMENT ) {
				$paymentForm = $this->context->smarty->fetch('module:dokuonecheckout/views/templates/hook/payment_channel_installment.tpl');
				$paymentOption->setCallToActionText("DOKU")
				->setForm($paymentForm)
				->setBinary(true);
			} else if ( intval($CHANNEL_OPTION) == 1 && intval($USE_INSTALLMENT) == 1 && intval($CHANNEL_CC) == 1 && $TOTAL_AMOUNT > $MIN_INSTALLMENT ) {
				$paymentForm = $this->context->smarty->fetch('module:dokuonecheckout/views/templates/hook/payment_channel_installment.tpl');
				$paymentOption->setCallToActionText("DOKU")
				->setForm($paymentForm)
				->setBinary(true);
			} else if ( intval($CHANNEL_OPTION) == 1 && intval($USE_INSTALLMENT) == 0 ) {
				$paymentForm = $this->context->smarty->fetch('module:dokuonecheckout/views/templates/hook/payment_channel.tpl');
				$paymentOption->setCallToActionText("DOKU")
				->setForm($paymentForm)
				->setBinary(true);
			} else {
